...
const DAKOTA_KEYS = ['kernel', 'systemd', 'bootc', 'mesa', 'nvidia', 'ogc-kernel', 'gnome', 'pipewire']
...
const dakotaRows = computed(() => {
  const packages = dakotaVersions.value?.packages
  if (!packages) {
    return []
  }
  return DAKOTA_KEYS
...
const wolvesDownloads = computed(() => [
  ...
  {
    title: t('TryBluefin.Wolves.Cards.Redacted'),
    description: t('TryBluefin.Wolves.Cards.RedactedDescription'),
    href: undefined,
    ...
    <div class="container">
      ...
      <section class="wolves-downloads" aria-labelledby="wolves-downloads-title">
        <div class="wolves-download-header">
          ...
          <p>{{ t('TryBluefin.Wolves.Description') }}</p>
        </div>
